...
def diabetes_mlops_pipeline():
    ...
    def download_dataset() -> dict:
        ...
        local_path = SOURCE_DATA_PATH
        bucket = "mlflow-artifacts"
        key = "datasets/diabetic_data.csv"

        if os.path.exists(local_path):
            print(f"Dataset ya existe en {local_path}, omitiendo descarga.")
            return {"source_path": local_path}

        os.makedirs(os.path.dirname(local_path), exist_ok=True)

        print(f"Descargando dataset desde MinIO s3://{bucket}/{key}...")
        ...
        print(f"Dataset descargado correctamente: {size_mb:.1f} MB en {local_path}")
